...
- **Licences** = Microsoft licences on the agent's own Entra account (`agent-teams-upn` tag, via Graph `User.Read.All`; never put the address in the registry) plus `fixedCosts` subscriptions from the registry
