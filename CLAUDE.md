...
- **Recordings** = Teams-native call recordings the agent started, indexed and served by the Presence bridge on its VM at the registry's `recordingsUrl` (`https://<agent>.knowall.ai/recordings`, token `RECORDINGS_TOKEN_<AGENT>` else `RECORDINGS_TOKEN`): `GET /api/agents/[id]/recordings` (list), `/recordings/status` (the _Recording_ chip), `/recordings/[rid]` (detail with per-turn transcript), `/recordings/[rid]/video` (302 to Teams' short-lived download URL resolved by the bridge, so the bytes never pass through the portal) and `/recordings/[rid]/transcript` (Teams' VTT). Admin-only: they are customer meeting content. `RECORDINGS_FIXTURE=1` or `?demo=1` serves sample data. Contract: agent-presence `docs/RECORDINGS.md`
- **Soul** = the agent's `SOUL.md` (`soulPath`, default `workspace/SOUL.md` then `SOUL.md`) rendered on its page
- **Training** = training runs the harness publishes to `knowall-ai/agent-training` (`TRAINING_REPO`): `runs/<agent-id>/*.json` per run on the agent's own `runs/<agent-id>` branch (default branch as fallback) and a root `curriculum.yaml` on the default branch, read with the server's GitHub token for an agent with a trusted registry entry. `src/lib/training.ts` parses both (a tiny YAML subset, no new dependency) and works out what is outstanding — never run, last run failed, or the last pass is older than the scenario's cadence. The listing fetches the newest 50 run files plus the latest file of each curriculum scenario, so the check is never fooled by truncation; a 404 is only "no runs" when the repo itself is readable
...
- Registry entries (brain URL, recordings URL, repo, boost script) only apply inside the scope they claim: their `subscriptionIds`, or the registry's own tenant — see `claimsResource` in `src/lib/agents/discover.ts`
...
| `RECORDINGS_TOKEN`         | Shared bearer token for agents' recordings API (Recordings tab); `RECORDINGS_TOKEN_<AGENT>` overrides it per agent                                | For the Recordings tab                            |
| `RECORDINGS_FIXTURE`       | `1` serves built-in sample recordings instead of the agent's VM (dev only)                                                                        | No                                                |
